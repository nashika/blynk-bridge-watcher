...
(function() {
  ...
  Board = (function(superClass) {
    ...
    function Board(parent, config, index) {
      ...
      var bridgeConfig, bridges, i, j, len, token;
      ...
      bridges = this._checkConfig(config, 'bridges', 'array');
      this.log('debug', "Construct Bridge objects was started.");
      this._bridges = {};
      i = 0;
      for (j = 0, len = bridges.length; j < len; j++) {
        bridgeConfig = bridges[j];
    ...
    Board.prototype._onInputVPin = function(param) {
      ...
      return (ref = this._bridges[bridgeName]).emit.apply(ref, [eventName, this._bridges[bridgeName]].concat(slice.call(eventArgs)));
    };
