  ...
  Base = (function(superClass) {
    ...
    Base.prototype._checkConfig = function(config, key, type, defaultValue) {
      ...
      if ((type === 'array' ? !Array.isArray(target) : !(typeof target === type))) {
        this.log('fatal', this.allKeyLabel(), "Check config. 'config." + key + "' type=" + (typeof target) + " is unexpected, expects " + type + ".");
        process.exit(1);
      }
      return target;
    };
    ...
    Base.prototype._initializeChildren = function(config, key, ChildClass) {
      var childConfig, childrenConfig, i, j, len;
      childrenConfig = this._checkConfig(config, key, 'array');
      this.log('debug', "Construct child '" + key + "' objects was started.");
      this[key] = {};
      i = 0;
      for (j = 0, len = childrenConfig.length; j < len; j++) {
        childConfig = childrenConfig[j];
        this[key][childConfig.name] = new ChildClass(this, childConfig, i++);
      }
    ...
    Base.prototype._initializeChildrenWithGenerator = function(config, key, ChildGenerator) {
      var childConfig, childrenConfig, generator, i, j, len;
      childrenConfig = this._checkConfig(config, key, 'array');
      this.log('debug', "Construct child '" + key + "' objects was started.");
      this[key] = {};
      i = 0;
      ...
      for (j = 0, len = childrenConfig.length; j < len; j++) {
        childConfig = childrenConfig[j];
        this[key][childConfig.name] = generator.generate(this, childConfig, i++);
      }
      return this.log('debug', "Construct child '" + key + "' objects was finished.");
    };
